...
const Dialogs:FC<DialogsPropsType> = ({DialogsPage, addMessage}) => {
    ...
    let messageElements = DialogsPage.messagesData.map(m => (<Message key={m.id} message={m.message}/>))
...
type OwnProps ={}
const AddMessageForm:FC<InjectedFormProps<NewMessageFormType,OwnProps>& OwnProps>  = ({handleSubmit}) => {
    const classes = useStyles();
    return (
        <form onSubmit={handleSubmit}>
            <div>
                {createField<NewMessageFormValuesKeysType>("Enter message", "newMessageBody", Textarea, undefined,[required, maxLength150] )}
